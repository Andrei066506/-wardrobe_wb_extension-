                ...
                for shift in range(len(pool)):
                    ...
                    if gender == "male":
                        ...
                        if not any(m in cand_name_low for m in ["женск", "женская", "женские", "женской", "женское", "для женщин", "для девочек"]):
                            continue
                    # Проверка стиля: утилитарная обувь не подходит для элегантных/офисных образов
                    if cat == "footwear" and anchor_style in ("elegant", "office"):
                        if any(kw in cand_name_low for kw in ["резин", "эва", "сапог резин", "резиновые", "утилитарн", "рабоч"]):
                            continue
                    picked = cand
                    break
                    ...
                    for cand in pool:
                        ...
                        if gender == "male":
                            ...
                            if not any(m in cand_name_low for m in ["женск", "женская", "женские", "женской", "женское", "для женщин", "для девочек"]):
                                continue
                        # Проверка стиля: утилитарная обувь не подходит для элегантных/офисных образов
                        if fc == "footwear" and anchor_style in ("elegant", "office"):
                            if any(kw in cand_name_low for kw in ["резин", "эва", "сапог резин", "резиновые", "утилитарн", "рабоч"]):
                                continue
...
def is_candidate_relevant(
    ...
    # Стиль + обувь: утилитарная/резиновая обувь не подходит для элегантных/офисных образов
    if candidate_category == "footwear" and anchor_style in ("elegant", "office"):
        utilitarian_shoes = any(kw in name_low for kw in ["резин", "эва", "сапог резин", "резиновые", "утилитарн", "рабоч"])
